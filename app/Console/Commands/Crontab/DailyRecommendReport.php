...
class DailyRecommendReport extends Command
{
    ...
    public function handle()
    {
       ...
       $rssCount = WechatWenzhangInfo::where('source_type',2)->where('status',2)->where('date_time','>=',date('Y-m-d 00:00:00'))->count();
       $wechatCount = WechatWenzhangInfo::where('source_type',1)->where('status',2)->where('date_time','>=',date('Y-m-d 00:00:00'))->count();
       ...
       event(new OperationNotify('今日截止目前入库文章：'.$rssCount.'(rss);'.$wechatCount.'(微信公众号);'.$wallstreetCount.'(华尔街见闻);'.$sapCount.'(sap blog)'));
    }
