...
class Feed extends Model
{
     ...
     */
    protected $fillable = [
        'user_id', 'user_id', 'top', 'feed_type','source_id','source_type','data','audit_status', 'is_anonymous'
    ];
        ...
        switch ($this->feed_type) {
            case self::FEED_TYPE_ANSWER_PAY_QUESTION:
                ...
                $data = [
                    ...
                    'answer_id' => $answer->id,
                    'tags'      => $question->tags()->select('id','name')->get()->toArray()
                ];
                break;
            ...
            case self::FEED_TYPE_COMMENT_FREE_QUESTION:
                ...
                $data['tags'] = $question->tags()->select('id','name')->get()->toArray();
                break;
