                    ...
                    <form role="form" name="editForm" method="POST" enctype="multipart/form-data" action="{{ route('admin.operate.pushNotice.update',['id'=>$notice->id]) }}">
                        ...
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                ...
                                <div class="radio">
                                    <label>
                                        <input type="radio" name="notification_type" value="1" @if($notice->notification_type===1) checked @endif /> 发现分享(仅限分享是链接的情形，url地址为发现分享id)
                                    </label>&nbsp;&nbsp;
                                    <label>
                                        <input type="radio" name="notification_type" value="2" @if($notice->notification_type===2) checked @endif /> 公告文章(外部url)
                                    </label>
                                    <label>
                                        <input type="radio" name="notification_type" value="3" @if($notice->notification_type===3) checked @endif /> app内页(内页地址，如：/c/35/java-base-at)
                                    </label>
                                </div>
                            </div>
